        ...
        <form action="{{ route('admin.attendance.update', $attendance->id) }}" method="POST">
            ...
            <div class="attendance-detail-card">
                ...
                @for($i = 0; $i < $breakCount; $i++)
                    @php
                        $startValue = optional($correctionBreaks->get($i))->requested_break_start
                            ? \Carbon\Carbon::parse($correctionBreaks->get($i)->requested_break_start)->format('H:i')
                            : optional($breaks->get($i))->break_start?->format('H:i') ?? '';
                        $endValue = optional($correctionBreaks->get($i))->requested_break_end
                            ? \Carbon\Carbon::parse($correctionBreaks->get($i)->requested_break_end)->format('H:i')
                            : optional($breaks->get($i))->break_end?->format('H:i') ?? '';
                    @endphp
                    <div class="attendance-detail__row">
                        <label class="attendance-detail__label">休憩{{ $i + 1 }}</label>
                        <input type="time" name="break_start_{{ $i + 1 }}" value="{{ $startValue }}" class="attendance-detail__input" {{ optional($attendance->correctionRequest)->status === 'pending' ? 'readonly' : '' }}>
                        〜
                        <input type="time" name="break_end_{{ $i + 1 }}" value="{{ $endValue }}" class="attendance-detail__input" {{ optional($attendance->correctionRequest)->status === 'pending' ? 'readonly' : '' }}>
                    </div>
                    <div class="form__error">
                        {{ $errors->first('break_start_' . ($i + 1)) ?: $errors->first('break_end_' . ($i + 1)) }}
                    </div>
                @endfor
                <div class="attendance-detail__row">
